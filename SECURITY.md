...
- Bugs in `auth.py`, `client.py`, `server.py`, or `scripts/authorize.py` that could leak OAuth tokens, the authorization code, account data, or transaction history outside the local MCP stdio channel.
...
- Configuration or packaging issues (`.gitignore` gaps, `pyproject.toml` misconfiguration, etc.) that could leak secrets.
